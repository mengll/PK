...
class Block {
  ...
  constructor(cube, scale = 1) {
    ...
    const material = new THREE.MeshLambertMaterial({color: 0xf2efbf})
    const model = new THREE.Mesh(geometry, material);

    model.position.z = 1 - height / 2;
    this.mesh.add(model);
  ...
  canvas() {
    ...
    // gradient.addColorStop( 0, '#fedcba' );
    // gradient.addColorStop( 1, 'rgba(255,255,255,1)' );
  ...
  texture() {
    if (Block._texture === null) {
      Block._texture = new THREE.CanvasTexture(this.canvas())
    }
    return Block._texture;
  }
...
class Bottle {
  ...
  stand(position) {
    ...
    this.mesh.quaternion.set(0,0,0,0);
...
export default class Game extends THREE.EventDispatcher {
  ...
  constructor() {
    ...
    this.scene.fog = new THREE.Fog(this.backgroundColor , 8.75, 15)
    //renderer
    ...
    const ground = new THREE.Mesh(new THREE.PlaneGeometry(FRUSTUM_WIDTH, FRUSTUM_HEIGHT), new THREE.MeshLambertMaterial({color: this.backgroundColor}));
      ...
      .do(() => {
        ...
        if (this.currentBlock.canHold(this.bottle.mesh.position)) {
          ...
          if ( !this.nextBlock.canHold(this.bottle.mesh.position) ) {
            ...
            this.addScore(1 + Math.min(5, this.combo))
  ...
  addScore(score) {
    if (score !== 0) {
      ...
      this.dispatchEvent({type: 'score', score: this.score});
    }
  }
  ...
  revive () {
    if (this.falling = true) {
      this.falling = false;
      this.bottle.stand(this.currentBlock.mesh.position.clone().add(new THREE.Vector3(0, 0, 1)));
      return true;
    }
    return false;
  }
