...
class DefaultMatching extends Component {
  ...
  async componentDidMount() {
    ...
    const {success, message} = await client.call('search_match', {user_limit: 2, game_id: gameId, uid: profile.uid})
    if (!success) {
...
class CreateMatching extends Component {
  ...
  componentDidMount() {
    loading(async () => {
      ...
      if (success) {
        share.share({
          image: window.location.origin + window.location.pathname + 'bottle-flip.jpg',
          url: window.location.origin + window.location.pathname + `#/invite/${gameId}/${this.room}`,
...
class JoinMatching extends Component {
  ...
  async componentDidMount() {
      ...
      const {success, message} = await client.call('join_room', {room: room, uid: profile.uid, game_id: gameId});
      if (!success) {
        Toast.fail(message, 2, () => {
          this.setState({
            back: true,
...
class Mathcing extends Component {
  ...
  componentWillUnmount() {
    ...
    if (this.state.matching && params.type === 'auto') {
      client.call('join_cancel', {uid: profile.uid, game_id: params.gameId})
    }
  }
  ...
  render() {
    const { type, room, gameId } = this.props.params;
...
export default class MatchingRoute extends Component {
  render() {
    ...
    return <AuthContext.Consumer>
        {
          ({profile}) => {
            return <Mathcing profile={profile} params={state}/>
          }
        }
